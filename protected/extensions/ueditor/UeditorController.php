...
class UeditorController extends CExtController{
    ...
    public function init(){
        ...
        $default = array(
            ...
            'imageUrlPrefix'=>Yii::app()->file->host,
            'videoUrlPrefix'=>Yii::app()->file->host,
        );
    ...
    protected function upload($fieldName,$config, $base64 = 'upload'){
        if($this->useQiniu)
        {
            $url = Yii::app()->file->upload($fieldName);
            // 水印有问题
            // $url = Yii::app()->file->waterMark($url);
            if(!empty($url))
            {
                $info = array(
                    'state' => 'SUCCESS',
        ...
        else
        {
            ...
            $info = $up->getFileInfo();
            if( $this->thumbnail&&$info['state']=='SUCCESS'&&in_array($info['type'],array('.png','.jpg','.jpeg','.bmp','.gif','.PNG','.JPG','.JPEG','.BMP','.GIF'))){
                $info['thumbnail'] = $this->imageHandle($info['url']);
            }
        }
    ...
    protected function show($result){
        ...
        if(isset($_GET["callback"])) {
            $callback = $purifier->purify($_GET['callback']);
     ...
     */
    protected function isGuest($count=0)
    {
        $prefix = array('_admin','_vip');
        if($count>(count($prefix)-1))
            return true;

        Yii::app()->user->setStateKeyPrefix($prefix[$count]);
        if(Yii::app()->user->isGuest)
            $this->isGuest($count+1);
        else
            return false;
    }
}
